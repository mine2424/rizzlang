...
- [ ] 12.1 API レート制限の実装
  - Edge Functions の認証検証ロジックを共通ユーティリティ関数として実装
  - `usage_logs` を使い 1ユーザーあたり 1分10リクエストを超えた場合に 429 エラーを返す
  - 未認証リクエストはすべて 401 で拒否する
  - _Requirements: 10.2, 10.3_

- [ ] 12.2 パフォーマンス最適化
  - Gemini API レスポンスを Flutter 側でスケルトンローディング表示
  - `conversations` テーブルの `(user_id, date)` 複合インデックスが存在することを確認
  - Firebase Performance Monitoring でネットワーク計測を有効化
  - `flutter build appbundle --release --obfuscate` でリリースビルド確認
...
- [ ] 13.1 ユニットテスト（`flutter_test`）
  - `DifficultyEngine.calcNextLevel()` のボーダー値テスト（noEditRate 0.8/0.4, avgRetries 0.5/2）
  - `VocabularyService` SRS スケジュール計算のテスト（SM-2 各フェーズ）
  - `StreakNotifier.updateStreak()` の継続/断絶両ケースのテスト
  - _Requirements: 5.2, 5.3, 6.4, 7.1, 7.2_

- [ ] 13.2 Widget テスト（`flutter_test`）
  - `MessageBubble` — ユーザー/キャラクター吹き出しの表示確認
  - `ReplyPanel` — 解説・スラングパネルの展開/折りたたみ確認
  - `StreakBar` — ストリーク数値の表示確認
  - _Requirements: 3.3, 3.7, 7.3_

- [ ] 13.3 統合テスト（`integration_test`）
  - `generateReply` — Edge Function モックで全フロー（認証・制限チェック・生成・保存）テスト
  - Stripe Webhook Handler — `checkout.session.completed` イベントで `plan=pro` になるテスト
  - シナリオ進捗 — `arc_day=7` 完了時に `arc_week+1` + `arc_day=1` になるテスト
