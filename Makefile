# ============================================================
# RizzLang — 開発ショートカット
# ============================================================
# 使い方:
#   make setup       初回セットアップ（依存インストール + ローカル起動）
#   make local-start Supabase ローカル起動 + マイグレーション適用
#   make local-stop  Supabase ローカル停止
#   make run         Flutter 実行（ローカルエミュレータ接続）
#   make run-device  Flutter を物理デバイスで実行（IP 要指定）
#   make test        テスト実行
#   make clean       キャッシュクリア
# ============================================================

.PHONY: setup local-start local-stop local-reset run run-device local-ip test clean help

# ── 物理デバイス接続時の Mac の LAN IP（変更してください）──
LOCAL_HOST ?= $(shell ipconfig getifaddr en0 2>/dev/null || hostname -I 2>/dev/null | awk '{print $$1}')

# ────────────────────────────────────────────────
# 初回セットアップ
# ────────────────────────────────────────────────
setup:
	@echo "🔧 RizzLang ローカル開発環境をセットアップします..."
	@command -v supabase >/dev/null 2>&1 || { echo "❌ Supabase CLI が必要です: brew install supabase/tap/supabase"; exit 1; }
	@command -v flutter >/dev/null 2>&1 || { echo "❌ Flutter SDK が必要です: https://flutter.dev/docs/get-started/install"; exit 1; }
	flutter pub get
	@$(MAKE) local-start
	@echo "✅ セットアップ完了！'make run' で起動してください"

# ────────────────────────────────────────────────
# Supabase エミュレータ 起動
# ────────────────────────────────────────────────
local-start:
	@echo "🚀 Supabase ローカルエミュレータを起動します..."
	supabase start
	@echo "📦 マイグレーションを適用します..."
	supabase db reset
	@echo ""
	@echo "╔════════════════════════════════════════════════╗"
	@echo "║  Supabase ローカル環境 起動完了！               ║"
	@echo "╠════════════════════════════════════════════════╣"
	@echo "║  API:    http://127.0.0.1:54321                ║"
	@echo "║  Studio: http://127.0.0.1:54323                ║"
	@echo "║  Email:  http://127.0.0.1:54324  (受信確認)    ║"
	@echo "║                                                 ║"
	@echo "║  テストユーザー: test@rizzlang.local           ║"
	@echo "║  パスワード:     test1234                       ║"
	@echo "╚════════════════════════════════════════════════╝"

# ────────────────────────────────────────────────
# Supabase エミュレータ 停止
# ────────────────────────────────────────────────
local-stop:
	@echo "🛑 Supabase ローカルエミュレータを停止します..."
	supabase stop

# ────────────────────────────────────────────────
# DB リセット（マイグレーション + シード再適用）
# ────────────────────────────────────────────────
local-reset:
	@echo "🔄 DB をリセットします（マイグレーション + シード再適用）..."
	supabase db reset
	@echo "✅ DB リセット完了"

# ────────────────────────────────────────────────
# Edge Functions をローカルで serve
# ────────────────────────────────────────────────
functions-serve:
	@echo "⚡ Edge Functions をローカルで起動します..."
	supabase functions serve --env-file supabase/.env.local

# ────────────────────────────────────────────────
# Flutter 実行（シミュレータ / エミュレータ）
# ────────────────────────────────────────────────
run:
	flutter run \
		--dart-define=ENV=local \
		--dart-define=RC_IOS_KEY=appl_dummy_local \
		--dart-define=RC_ANDROID_KEY=goog_dummy_local

# ────────────────────────────────────────────────
# Flutter 実行（物理デバイス）
# Mac の LAN IP に自動接続
# ────────────────────────────────────────────────
run-device:
	@echo "📱 物理デバイスで起動します（Supabase: http://$(LOCAL_HOST):54321）"
	flutter run \
		--dart-define=ENV=local \
		--dart-define=LOCAL_HOST=$(LOCAL_HOST) \
		--dart-define=RC_IOS_KEY=appl_dummy_local \
		--dart-define=RC_ANDROID_KEY=goog_dummy_local

# ────────────────────────────────────────────────
# Mac の LAN IP 確認
# ────────────────────────────────────────────────
local-ip:
	@echo "🌐 Mac の LAN IP: $(LOCAL_HOST)"
	@echo "   物理デバイス用: make run-device LOCAL_HOST=$(LOCAL_HOST)"

# ────────────────────────────────────────────────
# テスト実行
# ────────────────────────────────────────────────
test:
	flutter test

test-unit:
	flutter test test/unit/

test-widget:
	flutter test test/widget/

# VRT ゴールデンテスト（差分チェック）
test-golden:
	flutter test test/golden/

# VRT ゴールデンファイル生成（初回 or UI 変更後に実行）
test-golden-update:
	flutter test test/golden/ --update-goldens
	@echo "✅ ゴールデンファイルを更新しました。test/goldens/ の差分を目視確認してください。"

# E2E テスト（デバイス接続必須）
test-e2e:
	flutter test integration_test/e2e_test.dart -d $(or $(DEVICE_ID),$(shell flutter devices | grep -m1 'mobile\|simulator\|emulator' | awk '{print $$1}'))

test-coverage:
	flutter test --coverage
	genhtml coverage/lcov.info -o coverage/html
	open coverage/html/index.html

# ────────────────────────────────────────────────
# ビルド
# ────────────────────────────────────────────────
build-ios:
	flutter build ipa \
		--dart-define=ENV=production \
		--dart-define=SUPABASE_URL=$(SUPABASE_URL) \
		--dart-define=SUPABASE_ANON_KEY=$(SUPABASE_ANON_KEY) \
		--dart-define=RC_IOS_KEY=$(RC_IOS_KEY) \
		--obfuscate \
		--split-debug-info=build/debug-info/ios

build-android:
	flutter build appbundle \
		--dart-define=ENV=production \
		--dart-define=SUPABASE_URL=$(SUPABASE_URL) \
		--dart-define=SUPABASE_ANON_KEY=$(SUPABASE_ANON_KEY) \
		--dart-define=RC_ANDROID_KEY=$(RC_ANDROID_KEY) \
		--obfuscate \
		--split-debug-info=build/debug-info/android

# ────────────────────────────────────────────────
# クリーンアップ
# ────────────────────────────────────────────────
clean:
	flutter clean
	flutter pub get
	@echo "✅ クリーン完了"

# ────────────────────────────────────────────────
# ヘルプ
# ────────────────────────────────────────────────
help:
	@echo ""
	@echo "RizzLang 開発コマンド一覧"
	@echo "────────────────────────────────────────"
	@echo "  make setup          初回セットアップ"
	@echo "  make local-start    Supabase エミュレータ起動 + DB リセット"
	@echo "  make local-stop     Supabase エミュレータ停止"
	@echo "  make local-reset    DB リセット（マイグレーション + シード再適用）"
	@echo "  make functions-serve Edge Functions をローカルで起動"
	@echo "  make run            Flutter 実行（シミュレータ）"
	@echo "  make run-device     Flutter 実行（物理デバイス）"
	@echo "  make local-ip       Mac の LAN IP 確認"
	@echo "  make test                テスト実行（全件）"
	@echo "  make test-unit           ユニットテストのみ"
	@echo "  make test-widget         Widget テストのみ"
	@echo "  make test-golden         VRT ゴールデン差分チェック"
	@echo "  make test-golden-update  ゴールデンファイル生成・更新"
	@echo "  make test-e2e            E2E テスト（デバイス接続必須）"
	@echo "  make build-ios           iOS リリースビルド"
	@echo "  make build-android  Android リリースビルド"
	@echo "  make clean          キャッシュクリア"
	@echo ""
